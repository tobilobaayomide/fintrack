...
export default function TransactionItem({ transaction, onDelete, onEdit, compact }) {
  ...
  return (
    <div className="group flex items-center justify-between p-3 -mx-3 rounded-2xl hover:bg-slate-50 transition-colors duration-200">
      ...
      <div className="flex items-center gap-4 flex-shrink-0 pl-4">
        ...
        <span className={`text-sm font-bold tabular-nums text-right ${isIncome ? 'text-emerald-600' : 'text-slate-900'}`}>
          {isIncome ? "+" : "-"}{formatCurrency(transaction.amount)}
        </span>

        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 -mr-2 transition-opacity duration-200">
          <button 
