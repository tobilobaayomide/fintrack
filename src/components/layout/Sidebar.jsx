...
import { FiGrid, FiList, FiPieChart, FiChevronLeft, FiChevronRight, FiPlus } from "react-icons/fi"
import { FaNairaSign } from "react-icons/fa6"
import Button from "../ui/Button"

const NAV_ITEMS = [
  { path: "/dashboard",    label: "Dashboard",    icon: <FiGrid className="w-5 h-5" />     },
  { path: "/transactions", label: "Transactions", icon: <FiList className="w-5 h-5" />     },
  { path: "/budgets",      label: "Budgets",      icon: <FiPieChart className="w-5 h-5" /> },
]

export default function Sidebar({ activePath, onTabChange, currentMonth, onMonthChange, onAddClick }) {
  return (
    <aside className="w-64 bg-[#0a0f1c] border-r border-white/5 flex flex-col h-screen sticky top-0 overflow-y-auto">
      <div className="p-6 pb-2">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-xl font-bold text-white tracking-tight">Fin<span className="text-blue-500">Track</span></h1>
        </div>
